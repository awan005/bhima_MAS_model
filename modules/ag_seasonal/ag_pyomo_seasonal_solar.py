...
def ag_optimization_solar(input_df, solar_land):
    print()
    print('Starting Ag Optimization Run solar ++++')
    tic()
    ...
    def land_opt_solar_d_e(model, j, d): 
        return sum(model.land_opt_j_d_v['Solar', d] for d in model.d) <= solar_land/174
    
    
    model.land_opt_solar_d_e = Constraint(model.j, model.d, rule=land_opt_solar_d_e,
                                          doc='Solar cannot exceed solar land threshold (000ha)')
