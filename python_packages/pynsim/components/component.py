...
from python_packages.pynsim.history import Map
...
class Component(object):
    ...
    def validate_history(self):
        """
            Check whether this component's history can be exported
        """
        try:
...
class Container(Component):
    ...
    def add_link(self, link):
        """
            Add a single links to the network.
        """

        if link.start_node is None:
            raise Exception("A links must have a start node")
        if link.end_node is None:
            raise Exception("A links must have an end node")
        if link.name is None:
            default_link_name = link.start_node.name + " . " + link.end_node.name
            link.name = default_link_name
            logging.debug("No links name specified, defaulting to:%s", link.name)

        self.links.append(link)
        self.components.append(link)

        if link.name in self._link_map:
            raise Exception("An links with the name %s is already defined. Link names must be unique." % link.name)
    ...
    def get_link(self, link_name):
        """
            Get one links, by name. Returns None if links is not found.
        """
    ...
    def add_institution(self, institution):
        ...
        if institution.name in self._institution_map:
            raise Exception(
                "An institution with the name %s is already defined. Institutions names must be unique." % institution.name)
    ...
    def add_component(self, component):
        ...
        if component.name in self._component_map:
            raise Exception(
                "An component with the name %s is already defined. Component names must be unique." % component.name)
...
class Network(Container):
    ...
    def __init__(self, name, **kwargs):
        ...
        # Track the timing of the setup functions for each node,links and institution
            ...
            for l in self.links:
                if property_name in l.get_properties():
                    if len(nodes_to_plot) > 0:
                        logging.warn(
                            "WARNING: Some nodes have the same property %s as this links %s" % (property_name, l.name))
                    links_to_plot.append(l)
            for i in self.institutions:
                if property_name in i.get_properties():
                    if len(nodes_to_plot) > 0 or len(links_to_plot) > 0:
                        logging.warn(
                            "WARNING: Some nodes and links have the same property %s as this institution (%s)" % (
...
class Node(Component):
    ...
    def upstream_nodes(self):
        ...
        (nodes from where a links leads to this node).
        """
...
class Link(Component):
    """
        A links between two nodes.
    """
    # this never changes
    base_type = 'links'
    # This is updated in the __init__ function to the name of the links subclass
